...
/* global SpecialPowers, adapter:true, aContacts */
...
var GaiaDataLayer = {
  ...
  enableIMS: function() {
    let manager = window.navigator.mozMobileConnection ||
      window.navigator.mozMobileConnections &&
        window.navigator.mozMobileConnections[0];

    if (!manager.imsHandler.enabled) {
      waitFor(
        function() {
          console.log('IMS enabled');
          marionetteScriptFinished(true);
        },
        function() { return manager.imsHandler.enabled; }
      );
      manager.imsHandler.setEnabled(true);
    }
    else {
      console.log('IMS already enabled');
      marionetteScriptFinished(true);
    }
  },
  ...
  disableIMS: function() {
    ...
    if (manager.imsHandler.enabled) {
      waitFor(
        function() {
          console.log('IMS disabled');
          marionetteScriptFinished(true);
        },
        function() { return manager.imsHandler.enabled; }
      );
      manager.imsHandler.setEnabled(false);
    }
    else {
      console.log('IMS already disabled');
      marionetteScriptFinished(true);
    }
  },

  setPreferredIMSProfile: function(aProfile) {
    let manager = window.navigator.mozMobileConnection ||
      window.navigator.mozMobileConnections &&
        window.navigator.mozMobileConnections[0];
    console.log('setting preferred IMS profile to ' + aProfile);
    manager.imsHandler.setPreferredProfile(aProfile).then(function(value) {
      console.log('setting preferred IMS profile finished: ', value);
      marionetteScriptFinished(true);
    }, function(reason) {
      console.log('setting preferred IMS profile failed: ', reason);
      marionetteScriptFinished(false);
    });
  },

  setPreferredNetworkType: function(aType) {
    let manager = window.navigator.mozMobileConnection ||
      window.navigator.mozMobileConnections &&
        window.navigator.mozMobileConnections[0];
    console.log('setting preferred network type to ' + aType);
    let req = manager.setPreferredNetworkType(aType)

    req.onsuccess = function() {
      console.log('setting preferred network type finished: ' + req.readyState);
      marionetteScriptFinished(true);
    };

    req.onerror = function() {
      console.log('setting preferred network type failed');
      marionetteScriptFinished(false);
    };
  },
